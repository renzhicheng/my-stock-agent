...
# --- 4. 状态初始化 ---
if "cabinet_report" not in st.session_state:
    st.session_state.cabinet_report = ""
if "jinyiwei_report" not in st.session_state:
...
if st.button("🏮 宣：文武百官上朝议事", use_container_width=True):
    ...
    if st.session_state.cabinet_report:
        with st.status("锦衣卫正在审计资金异动...") as s2:
            try:
                j_p = f"你是一位顶级量化分析师。参考内阁分析：{st.session_state.cabinet_report}\n基于数据：{knowledge}\n请指出内阁遗漏的细节。"
